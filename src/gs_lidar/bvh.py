...
def build_bvh(box_min: torch.Tensor, box_max: torch.Tensor, leaf_size: int = 8) -> FlatBVH:
    ...
    def create(ids: torch.Tensor) -> int:
        index = len(nodes)
        node = {"min": box_min[ids].amin(0), "max": box_max[ids].amax(0), "left": -1, "right": -1,
                "first": -1, "count": 0}
        nodes.append(node)
    ...
    f = lambda key, dtype: torch.tensor([n[key] for n in nodes], dtype=dtype)
    return FlatBVH(torch.stack([n["min"] for n in nodes]).float(), torch.stack([n["max"] for n in nodes]).float(),
                   f("left", torch.int32), f("right", torch.int32), f("first", torch.int32),
                   f("count", torch.int32), torch.tensor(ordered, dtype=torch.int32))


def ray_aabb(origin: torch.Tensor, direction: torch.Tensor, lo: torch.Tensor, hi: torch.Tensor,
             near: float, far: float) -> bool:
    inv = torch.where(direction.abs() > 1e-12, direction.reciprocal(), torch.full_like(direction, float("inf")))
    t0, t1 = (lo-origin)*inv, (hi-origin)*inv
    return bool(torch.maximum(t0.minimum(t1).amax(), torch.tensor(near)) <=
                torch.minimum(t0.maximum(t1).amin(), torch.tensor(far)))


def bvh_candidates(bvh: FlatBVH, origin: torch.Tensor, direction: torch.Tensor, near: float, far: float) -> torch.Tensor:
    stack, result = [0], []
    while stack:
        node = stack.pop()
        if not ray_aabb(origin, direction, bvh.bbox_min[node], bvh.bbox_max[node], near, far):
            continue
