    ...
    def trace(
        ...
        for ray in range(count):
            ...
            t, q = ray_gaussian_peaks(
                origins[ray : ray + 1],
                directions[ray : ray + 1],
                scene.means[candidates],
                scene.scales[candidates],
                scene.rotations[candidates],
            )
            t, q = t[0], q[0]
            valid = (
                (t >= config.near)
                & (t <= config.far)
                & (q <= config.gaussian_sigma_cutoff**2)
            )
            ...
            for j in order:
                ...
                if a >= config.accumulated_alpha_threshold:
                    distance = t[valid][j]
                    ranges[ray], points[ray], hit[ray], ids[ray] = (
                        distance,
                        origins[ray] + distance * directions[ray],
                        True,
                        selected[j],
                    )
                    break
