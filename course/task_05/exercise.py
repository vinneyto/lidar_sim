...
def lidar_orientation(device: torch.device) -> torch.Tensor:
    # TODO: создайте quaternion поворота -90° вокруг X
...
def main() -> None:
    ...
    pose = LidarPose(orbit_position(device, 0.0), orientation)
    origins, directions = generate_rays(pose, config)
    print(f"Лучей: {directions.shape[0]}; начало первого: {origins[0].cpu().tolist()}")
    print(f"Quaternion wxyz: {orientation.cpu().tolist()}; норма: {orientation.norm():.4f}")
