...
def log_scan(step: int, pose: LidarPose, points: torch.Tensor) -> None:
    # TODO: запишите pose и points на timeline scan
...
def main() -> None:
    ...
    scan = simulator.scan(pose, create_config())
    log_scan(0, pose, scan.valid_points())
    print(f"Первый кадр: {int(scan.hit_mask.sum())} попаданий из {scan.hit_mask.numel()} лучей")
